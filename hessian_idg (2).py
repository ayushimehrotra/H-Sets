...
class HessianIntegratedDirectionalGradients:
    ...
    def make_attribution(self, input, index, max_elements, threshold):
        ...
        current_attribution_image = np.zeros(input.detach().cpu().numpy().shape)
    ...
    def attribute_set_ig(self, input, mask, steps=30):
        ...
        img = img/mag
        weight = math.comb(self.max_features, np.count_nonzero(mask))
        baseline = 0*input
        ...
        # print(np.linalg.norm(idg), weight)
        return np.linalg.norm(idg)

    def attribute(self, inputs, threshold=0.5, steps=30, k=5, max_samples=50):
        idgs = []
        for image in inputs:
