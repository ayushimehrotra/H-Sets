...
def Hessian_metrics(model, test_loader, threshold):
    ...
    for i, (x_batch, y_batch) in enumerate(test_loader):
        ...
        try:
            ...
            pass
    np.save("x_batches_densenet121", x_batches)
    np.save("y_batches_densenet121", y_batches)
    np.save("a_batches_densenet121", a_batches)
...
if __name__ == "__main__":
    ...
    model = model.to(device)
    model.load_state_dict(torch.load('model/densenet121.pth'))
    
    threshold = 0.50
    sparse, ROAD = Hessian_metrics(model, dataloaders["validation"], threshold)
    np.save("sparse_densenet121", sparse)
    np.save("ROAD_densenet121", ROAD)
