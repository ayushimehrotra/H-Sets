...
from archipelago.common_utils import get_efficient_mask_indices
...
def custom_mark_boundaries(
    ...
    if outline_color is not None:
        ...
        if marked.shape != desired_shape:
            marked = np.transpose(marked, (1,2,0))
...
def overlay_explanation(explanation, image, segments, outline_thickness=2):
    ...
    for i in reversed(range(min(len(sets_ranked), max_cnt))):
        ...
        temp += bound * explanation[sets_ranked[i]]
